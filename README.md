...
The simulated environment is a Mars-like terrain with an area of 100 x 100 m that is populated by thousands of rocks. We built this terrain by referring to the rock distribution photographed by TianWen-1. In each scene, we built imgae pair from the left and right virtual cameras, therefore **SynMars** can also support downstream vision missions like Stereo Matching and 3D reconstruction, etc. The distribution of all rocks in SynMars is displayed as follow, where 6 colored lines represent the traverse routes. 
